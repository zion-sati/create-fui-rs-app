...
function normalizePackageName(value: string): string {
  return value.trim().toLowerCase().replace(/[^a-z0-9._-]+/g, '-').replace(/^-+/, '').replace(/-+$/, '') || 'fui-rs-app';
}

function normalizeCrateName(value: string): string {
  return normalizePackageName(value).replace(/-/g, '_').replace(/[^a-z0-9_]+/g, '_') || 'fui_rs_app';
}
...
export function createProject(options: ScaffoldOptions): void {
  ...
  const packageName = normalizePackageName(options.projectName);
  const templateFiles = createTemplateFiles(template, {
    projectName: options.projectName,
    packageName,
    crateName: normalizeCrateName(packageName),
  });
  ...
  copyTemplateBinaryAssets(template, options.targetDirectory);
}
...
export function runCli(argv: readonly string[], cwd: string, logger: LoggerLike): number {
  ...
  const projectName = requestedPath === '.' ? 'fui-rs-app' : basename(resolve(cwd, requestedPath));

  try {
    createProject({ targetDirectory, projectName, template: parsed.template });
  } catch (error) {
